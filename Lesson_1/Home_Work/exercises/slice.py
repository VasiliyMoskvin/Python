...
def inside(s):
    # +++ ваш код +++
    # первый вариант создание 3-х списков: первый и последний символ; первая половина без первого элемента;
    # вторая половина без последнего элемента.
    lTemp = s.split()[1:int(len(s.split())/2)]
    lTemp.extend(s.split()[0::len(s.split())-1])
    lTemp.extend(s.split()[int(len(s.split())/2):-1])
    
    # второй вариант создание списка от 2-ого до предпоследнего элемента, и последующая вставка первого 
    # и последнего элемента в середину списка
    #lTemp = s.split()[1:-1]
    #lTemp.insert(int(len(lTemp)/2), s.split()[-1])
    #lTemp.insert(int((len(lTemp)-1)/2), s.split()[0])
    return " ".join(lTemp)
